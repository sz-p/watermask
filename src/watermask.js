export default class WaterMask {
	constructor(options) {
		let container = document.querySelector('#watermask') || document.body;
	...
	checkDom() {
		const canvas = document.getElementById('watermask-canvas');
	...
	startCheckDom() {
		if (this.options.checkDom) {
			setInterval(this.checkDom.bind(this), this.options.checkFrequency);
		}
	}
	initDom() {
	...
	initStyle() {
		this.canvas.id = 'watermask-canvas';
		...
		if (this.options.repeat) {
			for (let i = 0; i < this.options.cols; i++) {
				for (let ii = 0; ii < this.options.rows; ii++) {
					this.ctx.save();
					this.ctx.translate(
						this.options.startX + this.options.xGap * i,
						this.options.startY + this.options.yGap * ii
					);
